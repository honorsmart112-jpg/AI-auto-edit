...
import { AnalysisResult, SmartZoom, Subtitle, CutSegment } from '../types';
import { Scissors, type LucideIcon, FileText, ListVideo, Clock, Maximize, Pencil, Check, X, Trash2, AlertTriangle } from 'lucide-react';
...
type Tab = 'chapters' | 'cuts' | 'subtitles' | 'zooms';
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        ...
        {activeTab === 'zooms' && (
             ...
             analysis.zooms.map((zoom, i) => (
                <div key={i} className="p-3 bg-blue-900/10 hover:bg-blue-900/20 rounded transition-colors border border-blue-900/30 group">
                    ...
                    <div className="space-y-2">
                        <div className="flex items-center gap-2">
                            <span className="text-[10px] text-neutral-500 uppercase tracking-wider font-semibold">Target</span>
                            <select 
                                value={zoom.target}
                                onChange={(e) => onZoomUpdate(i, { target: e.target.value as any })}
                                className="flex-1 bg-neutral-950 border border-neutral-700 text-xs text-neutral-300 rounded px-2 py-1 focus:outline-none focus:border-blue-500"
                            >
        ...
        {activeTab === 'subtitles' && (
             analysis.subtitles.length === 0 ? <EmptyState label="No speech detected" /> :
...
const CutItem: React.FC<CutItemProps> = ({ cut, index, onSeek, onUpdate, onDelete }) => {
    ...
    return (
        <div className="p-3 bg-red-900/10 hover:bg-red-900/20 rounded transition-colors border border-red-900/30 group relative">
            <div className="flex justify-between items-start mb-1">
                <div onClick={() => onSeek(cut.start)} className="cursor-pointer">
...
const SubtitleItem: React.FC<SubtitleItemProps> = ({ sub, index, onSeek, onUpdate, onDelete }) => {
  ...
  if (isConfirming) {
      return (
        <div className="p-3 bg-neutral-800 border border-neutral-700 rounded shadow-inner space-y-2">
            <p className="text-sm text-neutral-300 text-center font-medium">Delete this subtitle?</p>
            <div className="flex gap-2">
                    <button 
                    onClick={confirmDelete}
                    className="flex-1 bg-red-600 hover:bg-red-500 text-white text-xs py-1.5 rounded font-medium transition-colors"
                >
                    Delete
                </button>
                <button 
                    onClick={() => setIsConfirming(false)}
                    className="flex-1 bg-neutral-900 hover:bg-neutral-800 text-neutral-300 text-xs py-1.5 rounded font-medium transition-colors border border-neutral-700"
                >
                    Cancel
                </button>
            </div>
  ...
  return (
    <div className={clsx(
        "p-3 rounded transition-colors border group relative",
        ...
        <div className="flex items-center justify-between mb-2">
            <span 
                className="text-xs font-mono text-emerald-500 hover:text-emerald-400 transition-colors cursor-pointer select-none" 
        ...
        {isEditing ? (
            <textarea 
                ...
                className="w-full bg-neutral-900 border border-neutral-700 rounded p-2 text-sm text-white focus:outline-none focus:border-blue-500 resize-none block"
                rows={Math.max(2, Math.ceil(sub.text.length / 35))}
                autoFocus
                placeholder="Enter subtitle text..."
            />
        ) : (
            <p className="text-sm text-neutral-300 italic whitespace-pre-wrap">
                "{sub.text}"
            </p>
        )}
    </div>
